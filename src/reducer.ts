...
export const rootReducer: Reducer<RootState> = (state = initialState, action) => {
  switch (action.type) {
    case UPDATE_TASKLIST: 
      return {
        ...state,
        taskList: [...state.taskList, { id: nanoid(), name: action.text, amount: 1, done: false }],
      }
    ...
    case UPDATE_TIME:
      switch(action.value) {
        case 'inc': state.time += ms('1m'); break;
        case 'dec': if(state.time > 0) state.time -= ms('1s'); break;
        default: state.time = ms(action.value);
      }
    ...
    case INCREASE_TOMATOCOUNT:
      return {
        ...state,
        tomatoCount: state.tomatoCount + 1,
      }
...
/*
if(action.value === 'inc'){
  state.time = state.time + ms('1m');
} else if(action.value === 'dec') {
  if(state.time > 0)
  state.time = state.time - ms('1s');
} else {
  state.time = ms(action.value);
}
...
if(action.value === 'inc'){
  ...
  if(task.tomato > 1)
  task.tomato--;
} else if(action.value === 'completed') {
  task.tomatoCompleted++;
} else if(action.value === 'done') {
  task.done = true;
} else {
  task.name = action.value;
}
...
export const updateTime: ActionCreator<AnyAction> = (id, value) => ({
  type: UPDATE_TIME,
  id,
  value,
})
...
export const rootReducer: Reducer<RootState> = (state = initialState, action) => {
  switch (action.type) {
    case UPDATE_TASKLIST: 
      return {
        ...state,
        taskList: [...state.taskList, { id: nanoid(), name: action.text, count: 1, time: ms('25m') }],
      }
    ...
    case UPDATE_TIME:
      for(const task of state.taskList) {
        if(task.id === action.id) {
          if(action.value === 'inc'){
            task.time = task.time + ms('1m');
          } else if(action.value === 'dec') {
            if(task.time > 0)
            task.time = task.time - ms('1s');
          } 
        }
      }
